...
export default defineEventHandler(async (event) => {
  ...
  const storageDir = join(process.cwd(), 'server', 'storage', 'uploads', 'landing')
  if (!existsSync(storageDir)) {
    mkdirSync(storageDir, { recursive: true })
  }

  // 6. Generate unique filename
  const originalName = fileField.filename || 'landing-asset.jpg'
  ...
  // 8. Return public URL
  return {
    url: `/api/uploads/landing/${fileName}`,
    filename: fileName
  }
})
