 ...
 * Экспорт выбранных терминов (и опционально их статей) в ZIP архив (с JSON внутри).
 */
...
export default defineEventHandler(async (event) => {
  ...
  let filename = `gativus-terms-export-${ts}.zip`
  if (extractedTermIds.length === 1) {
    filename = `term-${terms[0].slug}-${ts}.zip`
  } else if (idsParam) {
    filename = `terms-selected-${extractedTermIds.length}-${ts}.zip`
  } else if (query.search) {
    const safeSearch = (query.search as string).replace(/[^a-z0-9а-яё]/gi, '_').substring(0, 20)
    filename = `terms-search-${safeSearch}-${ts}.zip`
  } else {
    filename = `terms-filtered-${extractedTermIds.length}-${ts}.zip`
  }
  ...
  for (const t of terms) {
    addAssetToZip(t.image_url)
    addAssetToZip(t.video_url)
    addAssetToZip(t.presentation_path)
    addAssetToZip(t.presentation_path_ru)
    addAssetToZip(t.presentation_path_zh)
  }

  // Scan articles if included
  if (dump.articles) {
