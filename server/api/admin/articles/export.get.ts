 ...
 * Экспорт выбранных статей в JSON (версия 1.3).
 */
...
export default defineEventHandler(async (event) => {
  ...
  let filename = `gativus-articles-export-${ts}.json`
  if (articlesRaw.length === 1) {
    filename = `article-${articles[0].slug}-${ts}.json`
  } else if (idsParam) {
    filename = `articles-selected-${articlesRaw.length}-${ts}.json`
  } else if (query.search) {
    const safeSearch = (query.search as string).replace(/[^a-z0-9а-яё]/gi, '_').substring(0, 20)
    filename = `articles-search-${safeSearch}-${ts}.json`
  } else {
    filename = `articles-filtered-${articlesRaw.length}-${ts}.json`
  }
