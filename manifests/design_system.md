...
Проект строго поддерживает две темы (Светлая и Темная). Основной цветовой профиль опирается на монохромные и ахроматические шкалы TailwindCSS (`zinc`). 

### Светлая тема (Light Mode)
- **Базовый фон страницы:** `bg-white` (Чистая база).
- **Первичный акцент (Primary):** `sky-600` (#0284c7) — основной цвет для кнопок, активных ссылок и брендинга.
- **Первичный текст:** `text-gray-900` (Для мягкости).
- **Вторичный текст:** `text-gray-600` / `text-gray-500`.
- **Границы и разделители:** `border-gray-200` / `border-gray-100`.

### Темная тема (Dark Mode)
- **Базовый фон страницы:** `bg-zinc-900` (Глубокий графитовый серый).
- **Фон компонентов:** `bg-zinc-800`.
- **Первичный акцент (Primary):** `sky-500` (#0ea5e9) — иконки, активные состояния.
- **Глубокий акцент (Deep Accent):** `sky-900` (#0c4a6e) — для теней и контрастных элементов.
- **Первичный текст:** `text-gray-100` / `text-white`.
- **Вторичный текст:** `text-gray-400`.
- **Границы и разделители:** `border-zinc-800` / `border-zinc-700`.
...
- **Тени (Shadows):**
  ...
  - Структурные тени (Elevation): `shadow-sm` или `0 2px 15px rgba(0,0,0,0.05)` — применяется к Хедеру и основным блокам контента для легкого отделения от фона.
